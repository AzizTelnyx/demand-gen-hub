#!/usr/bin/env ts-node
/**
 * Creative Generator CLI
 *
 * Takes natural language briefs and generates production-ready creatives
 *
 * Usage:
 *   npm run generate-creative -- --prompt="A LinkedIn ad for healthcare targeting HIPAA compliance"
 *   npm run generate-creative -- --file=briefs/prompt-2.txt
 *   npm run generate-creative -- --batch=briefs/all-prompts.json
 *
 * Output:
 *   - Generated copy (headlines, descriptions, CTA)
 *   - Images in all required platform sizes
 *   - Organized in output/creatives/[campaign-name]/
 */
...
import { VISUAL_TEMPLATES, VisualCreativeData, VisualAssets } from "./visual-templates";
import { VISUAL_TEMPLATES_V2 } from "./visual-templates-v2";
import { VISUAL_TEMPLATES_V3 } from "./visual-templates-v3";
import { VISUAL_TEMPLATES_FINAL } from "./visual-templates-final";
import { selectAssets, detectProduct, detectIndustry } from "../src/lib/asset-selector";
...
const BRAND_GUIDE = {
  ...
  pillars: {
    trust: {
      keywords: ["HIPAA", "compliance", "secure", "SOC2", "PCI", "trust", "reliable"],
      visuals: ["compliance shield", "security badges", "lock icon", "donut chart 43%"],
      dataPoints: ["HIPAA-ready", "SOC 2 Type II", "99.999% uptime"],
    },
    infrastructure: {
      keywords: ["network", "vendor", "platform", "integration", "stack", "own", "build"],
      visuals: ["architecture diagram", "1 vs 5 vendors", "network topology"],
      dataPoints: ["1 platform", "Own network", "140+ countries", "Zero extra hops"],
    },
    physics: {
      keywords: ["latency", "ms", "faster", "speed", "performance", "real-time", "sub-"],
      visuals: ["latency chart", "speed comparison", "network hops"],
      dataPoints: ["<200ms", "Sub-500ms", "Co-located inference"],
    },
  },
  differentiators: [
    "Owns its own network (not a reseller like Twilio)",
    "Sub-200ms latency (inference co-located with telephony)",
...
/* ─── Brief Parser (AI-powered) ──────────────────────── */

async function parseBrief(promptText: string): Promise<any> {
...
- **Trust**: HIPAA, compliance, security, SOC2
- **Infrastructure**: Own network, 1 platform vs 5 vendors, integration
- **Physics**: Latency, speed, <200ms, performance
...
/* ─── Copy Generator (AI-powered) ────────────────────── */

async function generateCopy(brief: any): Promise<any> {
  const pillarInfo = BRAND_GUIDE.pillars[brief.pillar as keyof typeof BRAND_GUIDE.pillars] || BRAND_GUIDE.pillars.infrastructure;
...
**Rules:**
- No filler words (leading, best-in-class, innovative, etc.)
...
async function loadBrandConfig(): Promise<any> {
  try {
    const configPath = path.join(process.cwd(), "config", "brand-assets.json");
    const configContent = await fs.readFile(configPath, "utf-8");
    return JSON.parse(configContent);
  } catch (error) {
    console.warn("⚠️  Could not load brand-assets.json, using defaults");
    return {
      coreAssets: {
        logo: {
          primary: "brand-assets/assets/logo/green/telnyx-logo-wordmark-green.png"
        }
      }
    };
  }
...
async function generateHTML(
  ...
  // Select appropriate logo based on template background
  // V3 templates use LIGHT backgrounds (cream/pink tints) → black logo
  const logoPath = await selectLogo("#FEFDF5"); // Light background → black logo
  const logoBase64 = await imageToBase64(logoPath);

  // Use smart asset selector to pick best assets
  const selectedAssets = await selectAssets(brief);
  const detectedProduct = selectedAssets.detectedProduct;
  const detectedIndustry = selectedAssets.detectedIndustry;

  console.log(`   🎯 Detected: ${detectedProduct || 'general'} / ${detectedIndustry || 'no industry'}`);
  ...
  if (selectedAssets.background) {
    visualAssets.backgroundImage = await imageToBase64(selectedAssets.background);
    console.log(`   🖼️  Background: ${path.basename(selectedAssets.background)}`);
  }

  if (selectedAssets.productScreenshot) {
    visualAssets.productScreenshot = await imageToBase64(selectedAssets.productScreenshot);
    console.log(`   📸 Screenshot: ${path.basename(selectedAssets.productScreenshot)}`);
  }

  if (selectedAssets.industryPhoto) {
    visualAssets.industryPhoto = await imageToBase64(selectedAssets.industryPhoto);
    console.log(`   📷 Industry photo: ${path.basename(selectedAssets.industryPhoto)}`);
  }

  // Prepare visual creative data with product info
  const visualData: VisualCreativeData = {
    ...
    audience: brief.audience,
    platform: brief.platform,
    product: detectedProduct as any,
    industry: detectedIndustry as any,
  };

  const assets: VisualAssets = {
    logoBase64,
    backgroundImage: visualAssets.backgroundImage,
    productScreenshot: visualAssets.productScreenshot,
    industryPhoto: visualAssets.industryPhoto,
  };

  // PRIORITY LOGIC:
  // 1. Use-case focused (concierge, appointment, etc.) + industry → Use industry ABM template (shows relevant image)
  // 2. Voice AI Product (general product focus) → Use Voice AI product template with product colors
  // 3. Industry-only campaign → Use industry ABM template
  // 4. Fallback → Metric comparison
  ...
  if (isUseCaseFocused && detectedIndustry && visualAssets.industryPhoto) {
    // Use-case focused + industry → Show relevant industry image
    console.log(`   ✨ Using template: industry-abm (FINAL) for ${detectedIndustry} use-case`);
    return VISUAL_TEMPLATES_FINAL['industry-abm'](visualData, assets, width, height);
  } else if (isVoiceAIProduct && (visualAssets.productScreenshot || visualAssets.backgroundImage)) {
    // Voice AI Product Template - Matches real Telnyx ads
    console.log(`   ✨ Using template: voice-ai-product (FINAL) with ${detectedProduct} colors`);
    return VISUAL_TEMPLATES_FINAL['voice-ai-product'](visualData, assets, width, height);
  } else if (detectedIndustry && visualAssets.industryPhoto && !isVoiceAIProduct) {
    // Industry ABM Template - Matches real Telnyx ads
    console.log(`   ✨ Using template: industry-abm (FINAL) for ${detectedIndustry}`);
    return VISUAL_TEMPLATES_FINAL['industry-abm'](visualData, assets, width, height);
  } else {
    // Fallback to metric comparison template
    ...
    const fallbackAssets: TemplateAssets = { logoBase64 };
    console.log(`   ✨ Using template: metric-comparison (fallback)`);
    return TEMPLATES['metric-comparison'](creativeData, fallbackAssets, width, height);
  }
}
...
async function generateCreative(promptText: string) {
  ...
  // Step 3: Generate visuals for all sizes
  console.log(`\n🖼️  Generating visuals for ${brief.platform}...`);
  ...
  const outputDir = path.join(
    ...
    "output",
    "creatives",
    `${brief.platform}-${brief.pillar}-${brief.audience.replace(/\s+/g, "-").toLowerCase()}-${Date.now()}`
  );
  await fs.mkdir(outputDir, { recursive: true });

  for (const size of sizes) {
    const html = await generateHTML(brief, copy, size.width, size.height);
    const outputPath = path.join(outputDir, `${size.label}.png`);
    await convertHTMLtoPNG(html, outputPath, size.width, size.height);
  }

  // Step 4: Save copy as JSON
  ...
  console.log(`   2. Review copy in copy.json`);
  console.log(`   3. Upload PNGs to ${brief.platform}`);
  console.log("\n" + "━".repeat(60) + "\n");
}
...
Usage:
  ...
  npm run generate-creative -- --file=path/to/brief.txt

Examples:
  npm run generate-creative -- --prompt="A LinkedIn ad for healthcare targeting HIPAA compliance"
  npm run generate-creative -- --prompt="A StackAdapt native ad for developers"

Output:
  - Copy (headlines, descriptions, CTA)
  - Images in all platform-required sizes
  - Organized in output/creatives/[campaign]/
    `);
    process.exit(0);
  }
  ...
  if (promptIndex !== -1 && args[promptIndex + 1]) {
    promptText = args[promptIndex + 1];
  } else if (fileIndex !== -1 && args[fileIndex + 1]) {
    promptText = await fs.readFile(args[fileIndex + 1], "utf-8");
  } else {
